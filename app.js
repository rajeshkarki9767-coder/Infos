(function() {
  ...
  function openTermsModal() {
    openModal(`
      ...
      <div class="modal-body" style="line-height:1.7;font-size:13px;color:var(--text-secondary);">
        ...
        <h4 class="guide-h3" style="margin-top:0;">1. Local-first storage</h4>
        <p>Infos stores all your data on your device. We do not collect, transmit, or process your data on any server unless you explicitly enable sync.</p>
        <h4 class="guide-h3">2. Your account</h4>
        <p>You're responsible for keeping your sign-in credentials safe. Forgotten credentials cannot be recovered by us — there is no central account directory.</p>
        ...
        <h4 class="guide-h3">4. Encryption</h4>
        <p>If you enable end-to-end encryption, your master password is never stored. We cannot recover encrypted business passwords if you forget it.</p>
        <h4 class="guide-h3">5. Acceptable use</h4>
        ...
        <p>Infos is provided as-is. We make no warranties about availability, accuracy, or fitness for any purpose.</p>
        <h4 class="guide-h3">7. Changes</h4>
  ...
  function openPrivacyPreviewModal() {
    openModal(`
      ...
      <div class="modal-body" style="line-height:1.7;font-size:13px;color:var(--text-secondary);">
        <p style="margin:0 0 12px;color:var(--text-primary);"><strong>What Infos collects: nothing.</strong></p>
        <p>All data — your account, businesses, items, tags, attachments, preferences — is stored on your device in IndexedDB. Nothing leaves your device unless you explicitly export it or turn on backend sync.</p>
        <h4 class="guide-h3">Owner sign-in</h4>
        <p>Your account email, name, and password are stored only on your device. There is no cloud account directory.</p>
        ...
        <h4 class="guide-h3">Optional encryption</h4>
        <p>Business passwords can be encrypted with a master password (Settings → Management → Enable encryption). The master password is never stored — we cannot recover it.</p>
        <h4 class="guide-h3">Optional sync</h4>
        <p>Disabled by default. When enabled, your data is uploaded to a backend you configure. Encrypted fields remain ciphertext to the backend.</p>
      </div>
      <div class="modal-foot"><div></div><button class="btn-primary" id="m-cancel">Got it</button></div>
    `);
    $('#m-close').onclick = closeModal;
  ...
  function renderAbout(c) {
    c.innerHTML = viewOnlyBanner() + `<div style="max-width:480px;">
      ...
      <p style="font-size:14px;color:var(--text-secondary);line-height:1.7;">Manage multiple businesses, their notices, credentials, and accounts. View-only sharing for teams. Local-first with optional end-to-end encryption.</p>
    </div>`;
  }
  function renderPrivacy(c) {
    ...
    if (bizCtx) {
      c.innerHTML = viewOnlyBanner() + `<div style="max-width:600px;">
        ...
        <div class="guide-section">
          <h3 class="guide-h3">Local storage</h3>
          <p class="guide-p">All data stays on the device. Nothing is transmitted unless the owner explicitly enables sync.</p>
        </div>
      </div>`;
    } else {
      c.innerHTML = `<div style="max-width:600px;">
        <h2 class="page-h2">Privacy policy</h2>
        <p class="page-lead">Infos is local-first. Your data stays on your device.</p>
        <div class="guide-section">
          <h3 class="guide-h3">Local-first storage</h3>
          <p class="guide-p">All data — businesses, items, tags, preferences, attachments — is stored in IndexedDB on your device. Nothing leaves your device unless you explicitly export, share, or enable a sync backend.</p>
        </div>
        <div class="guide-section">
          <h3 class="guide-h3">End-to-end encryption</h3>
          <p class="guide-p">Optional. When enabled, business passwords are encrypted with a master password that's never stored. Only a verifier hash is persisted. Forget the master password and the data is unrecoverable — by design.</p>
        </div>
        <div class="guide-section">
        ...
        <div class="guide-section">
          <h3 class="guide-h3">Sync (optional)</h3>
          <p class="guide-p">Disabled by default. If you turn on a backend adapter, data is uploaded to that backend on every change. Encrypted business passwords remain ciphertext to the backend.</p>
        </div>
        <div class="guide-section">
          <h3 class="guide-h3">Your rights</h3>
          <p class="guide-p">Clear all data from Settings → Backup, or your browser's site-data controls.</p>
        </div>
      </div>`;
    }
  }
      ...
      const guideHTML = bizCtxLocal
        ...
        : `<div class="guide-section">
          ...
          <div class="guide-section">
            ...
            <p class="guide-p"><strong>Local-first.</strong> All data stays in IndexedDB on your device. Nothing leaves unless you explicitly export or enable sync.</p>
            <p class="guide-p"><strong>End-to-end encryption.</strong> Optional, opt-in from the Management tab. Business passwords get encrypted with a master password that's never stored.</p>
            ...
            <p class="guide-p"><strong>Sync.</strong> Disabled by default. If enabled, data uploads to the configured backend on every change.</p>
            <p class="guide-p"><strong>Your rights.</strong> Clear all data from Backup tab, or browser site-data controls.</p>
          </div>`;
      return `
